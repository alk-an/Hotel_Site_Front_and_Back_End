<?php
  ...
  function send_mail($email, $token, $type){
    ...
    try {
        ...
        $mail->Username   = 'alkanastasiadis@gmail.com';                     //SMTP username
        $mail->Password   = 'zxcovelzhbkupjbm';                               //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

        //Recipients
        $mail->setFrom(PHPMAILER_EMAIL, PHPMAILER_NAME);
        $mail->addAddress($email, 'esi');     //Add a recipient
        ...
        //$var = <a href='".SITE_URL."email_confirm.php?email=$email&token=$token"."'>Click Me</a>;

        //Content
        $mail->isHTML(true);                                  //Set email format to HTML
        $mail->Subject = "$subject";
        $mail->Body    = "Click the link to $content: <br> <a href='".SITE_URL."$page?$type&email=$email&token=$token"."'>Click Me</a> ";
        $mail->AltBody = "Click the link to $content: <br> <a href='".SITE_URL."$page?$type&email=$email&token=$token"."'>Click Me</a> ";
  ...
  if(isset($_POST['register'])){
    ...
    send_mail($data['email'],$token,"email_confirmation");
